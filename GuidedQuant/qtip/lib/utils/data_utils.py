...
class SimpleDatasetBlockHess(Dataset):
...
def split_data(X, Y, block_hess_data, args):
    split = int(len(X) - args.ft_valid_size)
    glog.info(f'using {split} training seqs, {len(X) - split} validation seqs')
    if block_hess_data is not None:
        train_ds = SimpleDatasetBlockHess(X[:split], Y[:split], block_hess_data[:split])
        valid_ds = SimpleDatasetBlockHess(X[split:], Y[split:], block_hess_data[split:])
    else:
