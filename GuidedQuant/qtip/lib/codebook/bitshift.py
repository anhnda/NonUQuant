    ...
    def __init__(self,
        ...
        if decode_mode == 'lut':
            ...
            if tlut is None:
                assert tlut_bits > 0
                if V == 2:
                    fname = f'../cache/qtip_lut/kmeans_{tlut_bits}_{V}.pt'
...
class BitshiftLinear(nn.Module):
    ...
    def forward(self,
        ...
        if mode == 'train-fixW':
            ...
            if bs <= 8 and self.has_kernel:
                wrapper = getattr(
                    torch.ops.quip_lib,
                    f"decompress_matvec_qtip_{m}_1_{n}_{self.cb.K}")
                if bs == 1:
                    x = wrapper(trellis, x, self.cb.tlut)
                else:
                    outs = []
                    for i in range(bs):
                        outs.append(wrapper(trellis, x[i].unsqueeze(0), self.cb.tlut))
