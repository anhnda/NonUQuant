...
parser.add_argument('--ft_train_recons', action='store_true')
...
parser.add_argument('--g_sal', default=4, type=int)
parser.add_argument('--block_hess_path', default=None, type=str)
parser.add_argument('--tokens_path', default=None, type=str)
parser.add_argument('--pin_memory', action='store_true')
parser.add_argument('--sub_in_hess_path', default=None, type=str)
parser.add_argument('--sub_use_saliency', action='store_true')
parser.add_argument('--sub_g_sal', default=4, type=int)
...
def main(args):
    ...
    if args.tokens_path is not None:
        tokens = torch.load(args.tokens_path) # list of N number of T tensor
        devset = torch.stack(tokens, dim=0).to(torch.int64) # N x T
        assert args.devset_size <= devset.shape[0], f'{args.devset_size} > {devset.shape[0]}'
        devset = devset[:args.devset_size]
    else:
    ...
    for i in range(len(model.model.layers)):
        ...
        for j in range(devset.shape[0] // args.batch_size):
            utils.clean()
