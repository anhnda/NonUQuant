...
def main(args):
    ...
    if args.tokens_path is not None and args.tokens_path !='None':
        print("Loading devset from ", args.tokens_path)
        tokens = torch.load(args.tokens_path) # list of N number of T tensor
        devset = torch.stack(tokens, dim=0).to(torch.int64) # N x T
        devset = devset[:args.devset_size] # Ndev x T
    else:
        print("Sampling devset")
    ...
    for name, module in quant_model.named_modules():
        if isinstance(module, QuantizedLinear):
            ...
            if (args.ft_train_lut or args.ft_train_recons) and not module.has_kernel:
