...
def ptq_model(args, model, model_args=None):
    ...
    if args.w_bits < 16:
        ...
        if args.load_qmodel_path:  # Load Quantized Rotated Model
            ...
            if args.input_tokens_path is not None:
                assert os.path.exists(args.input_tokens_path), "Input tokens path does not exist!"
                trainloader = torch.load(args.input_tokens_path)
                print("Loaded input tokens from ", args.input_tokens_path)
            else:
            ...
            if isinstance(trainloader[0], torch.Tensor):
                ...
                print("Reformatting input tokens to tuple + Unsqueeze")
            ...
            if args.guided:
                ...
                quantizers = gptq_guided_utils.gptq_fwrd(model, trainloader, "cuda:0", args)
            else:
